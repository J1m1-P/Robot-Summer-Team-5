...
void test_integral_accumulates_normally_while_target_nonzero() {
    ...
    wheel_velocity_pi_update(pi, cfg, 1.0f, 0.5f, 0.1f, duty);
...
void test_integral_resets_immediately_when_target_becomes_zero() {
    ...
    for (int i = 0; i < 20; i++) {
        wheel_velocity_pi_update(pi, cfg, 1.0f, 0.9f, 0.1f, duty);
    }
    ...
    wheel_velocity_pi_update(pi, cfg, 0.0f, 0.9f, 0.1f, duty);
...
void test_integral_stays_zero_while_target_remains_zero_and_wheel_still_coasting() {
    ...
    float measured = 0.9f;
    for (int i = 0; i < 10; i++) {
        wheel_velocity_pi_update(pi, cfg, 0.0f, measured, 0.1f, duty);
        TEST_ASSERT_EQUAL_FLOAT(0.0f, pi.integral);
...
void test_integral_resumes_accumulating_once_target_goes_nonzero_again() {
    ...
    wheel_velocity_pi_update(pi, cfg, 0.0f, 0.5f, 0.1f, duty); // stop -> reset to 0
    TEST_ASSERT_EQUAL_FLOAT(0.0f, pi.integral);

    wheel_velocity_pi_update(pi, cfg, 1.0f, 0.5f, 0.1f, duty); // moving again
    TEST_ASSERT_TRUE(pi.integral > 0.0f);
}
