...
typedef enum {
    ...
    // CMD_TOWER_Z/X are single bidirectional commands (direction comes from
    // the command's signed value, not the opcode), so there's one completion
    // detail per axis rather than a raised/lowered or left/right pair.
