...
static uint8_t calculate_checksum(uint8_t version, uint8_t message_type, uint8_t payload_len,
...
static void parser_process_byte(UartLink *link, uint8_t byte) {
    ...
    switch (parser->state) {
        ...
        case PACKET_PARSE_TYPE:
            ...
            parser->checksum_accumulator =
                checksum_update(parser->checksum_accumulator, byte);
...
esp_err_t uart_link_init(UartLink *link, const UartLinkConfig *config) {
    ...
    if (config->rx_buffer_size > INT_MAX || config->tx_buffer_size > INT_MAX) {
